    ...
    fun handleContentModeRequest(client: SocketIOClient, data: String, ack: AckRequest) {
        ...
        if (player == null) {
            ...
            if (onlinePlayers.isEmpty()) {
                ack.sendResult(Result.failure(Exception("没有玩家在线，无法启动内容模式")))
                return
            }
        ...
        try {
            ...
            if (mode !is ContentMode) {
                ack.sendResult(Result.failure(Exception("内容模式类 ${request.contentModeClassName} 没有实现 ContentMode")))
                return
            }
            ...
            ack.sendResult(Result.failure(Exception("找不到内容模式类 ${request.contentModeClassName} 的构造函数 (ContentContext、Player)")))
            e.printStackTrace()
            return
        }
    }
