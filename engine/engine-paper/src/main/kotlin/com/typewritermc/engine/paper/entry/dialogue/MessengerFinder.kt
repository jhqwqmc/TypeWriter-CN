...
private val confirmationKeyString by config(
    ...
    |确认对话选项时应按下的键。
    |可能的值: ${ConfirmationKey.entries.joinToString(", ") { it.name }}
""".trimMargin()
)
...
private val typingDurationTypeString by config(
    ...
    |应该使用的输入时长类型。
    |可能的值: ${TypingDurationType.entries.joinToString(", ") { it.name }}
""".trimMargin()
)
...
val typingDurationType: TypingDurationType by reloadable {
    ...
    if (type == null) {
        plugin.logger.warning("无效的输入时长类型 '$typingDurationTypeString'。使用默认类型 '${TypingDurationType.TOTAL.name}' 代替。")
